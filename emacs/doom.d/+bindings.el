...
(map! [remap evil-jump-to-tag] #'projectile-find-tag
      ...
      (:leader
        ...
        ;;       :desc "Format buffer"           :n  "f"  #'prettier-js
        ;;       :desc "Rename symbol"           :n  "rn" #'tide-rename-symbol
        ;;       :desc "Fix"                     :n  "x"  #'tide-fix
        ;;       :desc "Nav"                     :n  "n"  #'tide-nav
        ;;       :desc "Jump to definition"      :n  "d"  #'+lookup/definition
        ;;       :desc "Jump to references"      :n  "D"  #'+lookup/references
        ;;       ))
        ;;   (:after haskell-mode
        ;;     (:map haskell-mode-map
        ;;       :desc "Jump to definition"      :n  "d"  #'intero-goto-definition
        ;;       ;; uses-at puts the buffer in read-only-mode and i don't know how to exit it
        ;;       ;; :desc "Find Usages"             :n  "D"  #'intero-uses-at
        ;;       :desc "Rename"                  :n  "r"  #'hare-refactor-rename
        ;;       :desc "Open REPL"               :n  "I"  #'intero-repl-load
        ;;       :desc "Apply Intero suggestions"       :n  "X"  #'intero-apply-suggestions
        ;;       :desc "Apply Hlint suggestions"        :n  "x"  #'hlint-refactor-refactor-at-point
        ;;       :desc "Insert type annotation"  :n  "T"  (lambda ()
        ;;                                                  (interactive)
        ;;                                                  ;; https://stackoverflow.com/questions/12827887/emacs-universal-argument-c-u-in-a-function
        ;;                                                  (let ((current-prefix-arg 4))
        ;;                                                    (call-interactively 'intero-type-at)))
        ;;       )
        ;;     )
        ;;   ;; (:after intero-mode
        ;;   ;;   (:map intero-mode-map
        ;;   ;;     :desc "Insert type annotation" :n "T" #'intero-type-at
        ;;   ;;     )
        ;;   ;;   )
        ...
        ;;   (:after reason-mode
        ;;     (:map reason-mode-map
        ;;       :desc "Format buffer" :n "f" #'refmt
        ;;       )
        ;;     )
        ;;   (:after web-dev-mode
        ;;     (:map tide-mode-map
        ;;       :desc "Format buffer"           :n  "f" (lambda()
        ;;                                                 (interactive)
        ;;                                                 (funcall g/tide-format-fn))
        ;;       (:desc "Rename" :prefix "r"
        ;;         :desc "Symbol"                :n  "n" #'tide-rename-symbol
        ;;         :desc "Element"               :n  "e" #'web-mode-element-rename)
        ;;       :desc "Fix"                     :n  "x" #'tide-fix
        ;;       :desc "Nav"                     :n  "n" #'tide-nav))

        ;;   (:after graphql-mode
        ;;     (:map graphql-mode-map
        ;;       :desc "Prettier" :n "f" #'prettier-js))

        ;;   (:after elm-mode
        ;;     (:map elm-mode-map
        ;;       :desc "Format" :n "f" #'elm-mode-format-buffer
        ;;       :desc "Type" :n "t" #'elm-oracle-type-at-point)
        ;;     )
        ;;   )

        (:desc "configuration" :prefix "C"
          :desc "Bindings" :n "B" #'g/edit-bindings
          :desc "Edit Doom" :n "D" #'g/edit-doom-config
          :desc "Edit" :n "E" #'g/edit-config
          :desc "Find" :n "F" #'+private/find-in-config
          :desc "Reload" :n "R" #'g/reload-config)
        ...
        (:desc "file" :prefix "f"
          ...
          :desc "Make directory" :n "d" #'make-directory
        ...
        (:desc "toggle" :prefix "t"
          ...
          :desc "Which Key mode"         :n "w" #'which-key-mode
      ...
      (:after evil-mc
        :map evil-mc-key-map
        :nv "C-S-a" #'evil-mc-make-all-cursors
        ...
        :nv "C-g" #'evil-mc-make-and-goto-first-cursor
        :nv "C-S-G" #'evil-mc-make-and-goto-last-cursor)
