# Enables sudo authentication with Touch ID
# copied from https://github.com/LnL7/nix-darwin/pull/662 
# The current security.pam implementation doesn't provide an option to use pam_reattach,
# which is necessary for making this work inside tmux.
{ config , lib , pkgs , ... }:
with lib; let
  cfg = config.custom.security.pam;
in
{
  options = {
    custom.security.pam = {
